...
export const messageService = {
  async createMessage(
    data: Pick<IMessage, "name" | "email" | "phone" | "message">,
    req: Request,
  ): Promise<IMessage> {
    const fmessage = await Message.create({
      ...data,
      ipAddress: req.ip,
      userAgent: req.get("User-Agent"),
    });

    const { subject, html } = emailTemplates.contactNotification(fmessage);
    sendEmail({ to: process.env.SMTP_USER ?? "", subject, html }).catch(
      (err: unknown) =>
