...
import type { ISubscriber, PaginatedResult, SubscriberStats, BaseQuery } from '../types/index.js';
...
export const subscriberService = {
  async subscribe(email: string, source = 'website'): Promise<ISubscriber> {
    ...
    if (existing) {
      if (existing.isActive) throw new ConflictError('This email is already subscribed.');
    ...
    const { subject, html } = emailTemplates.subscriptionConfirm(unsubscribeUrl);
  ...
  async getSubscribers(
    ...
    return { data: subscribers, meta: buildPaginationMeta({ total, page, limit }) };
  },
